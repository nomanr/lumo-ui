...
import com.nomanr.compose.ui.foundation.ProvideContentColorTextStyle
...
internal fun CommonDecorationBox(
    ...
    TextFieldTransitionScope.Transition(
            ...
            if (placeholder != null && transformedText.isEmpty() && placeholderAlphaProgress > 0f) {
                @Composable { modifier ->
                    Box(modifier.alpha(placeholderAlphaProgress)) {
                        Decoration(
                            contentColor = placeholderColor, typography = AppTheme.typography.input, content = placeholder
                        )
                    }
                }
            } else null
        ...
        val decoratedPrefix: @Composable (() -> Unit)? = if (prefix != null) {
            @Composable {
                Decoration(
                    contentColor = prefixColor, typography = AppTheme.typography.input, content = prefix
                )
            }
        } else null
        ...
        val decoratedSuffix: @Composable (() -> Unit)? = if (suffix != null) {
            @Composable {
                Decoration(
                    contentColor = suffixColor, typography = AppTheme.typography.input, content = suffix
                )
            }
        } else null
